...
class ApiOptimizer {
    ...
    async optimizeApiCall(url, options, originalFetch) {
        ...
        if (cached && Date.now() - cached.timestamp < this.cacheTimeout) {
            // Reduzir logs de cache para evitar spam
            if (!this.lastCacheLog || Date.now() - this.lastCacheLog > 5000) {
                console.log(`⚡ ApiOptimizer: Retornando dados do cache para ${url}`);
                this.lastCacheLog = Date.now();
            }
        ...
        const requestPromise = originalFetch(url, options).then(async response => {
            ...
            if (response.ok) {
                try {
                    ...
                    // Reduzir logs de cache para evitar spam
                    if (!this.lastCacheStoreLog || Date.now() - this.lastCacheStoreLog > 5000) {
                        console.log(`⚡ ApiOptimizer: Dados de ${url} armazenados no cache`);
                        this.lastCacheStoreLog = Date.now();
                    }
                } catch (error) {
                    console.warn(`⚠️ ApiOptimizer: Erro ao fazer cache de ${url}:`, error);
                }
            }
